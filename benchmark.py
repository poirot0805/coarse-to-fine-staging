...
def get_linear_interpolation(start, end, length:int):
    """
...
def get_linear_interpolation2(start, end, length):
    """
    Get linear interpolation between start and end.
...
def _get_interpolated_local_pos_quat(positions, rotations, seq_slice):
    ...
    # Lerp root position
    pos = get_linear_interpolation2(
        positions[..., context_slice, :, :],
        positions[..., target_slice, :, :], n_trans)
