...
def normalize_torch(tensor, dim:int=-1, eps:float=1e-5):
    """
    ...
    """
    return F.normalize(tensor, p=2.0, dim=dim, eps=eps)
...
def matrix6D_to_9D_torch2(mat):
    ...
    if len(mat.shape)==3:
        mat = mat.reshape(mat.shape[0],mat.shape[1], 3, 2)
    else:
        mat = mat.reshape(mat.shape[0], mat.shape[1],mat.shape[2],3, 2)
...
if __name__ == "__main__":
    scripted_fn2 = torch.jit.script(matrix6D_to_9D_torch)
    print(scripted_fn2.code)
    scripted_fn2.save("data_utils_matrix6D_to_9D_torch.pt")
