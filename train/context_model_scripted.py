...
def get_train_stats(config:Dict[str,str], use_cache:bool=True, stats_folder:str=None,
                    dataset_name:str="train_stats"):
    context_len = 1
...
def get_train_stats_torch(config:Dict[str,str], dtype:torch.dtype, device:torch.device,
                          use_cache:bool=True, stats_folder:str=None,
                          dataset_name:str="train_stats"):
...
def get_midway_targets(seq_slice:slice, midway_targets_amount:int, midway_targets_p:float):
    targets = set()
...
@torch.jit.script
def get_data_mask(window_len:int, d_mask:int, constrained_slices,
                  context_len:int, target_idx:int, 
                  midway_targets:Optional[torch.Tensor]):
    # 0 for unknown and 1 for known
    # print("get_data_mask_constraint:{}".format(constrained_slices))
    constrained_slices2 = torch.jit.annotate(List[List[int]], constrained_slices.tolist())

    data_mask = torch.zeros((window_len, d_mask), device="cpu", dtype=torch.float32)
...
@torch.jit.script
def get_keyframe_pos_indices(window_len:int, seq_slice_start:int,seq_slice_stop:int):
    dtype = torch.float32
    device="cpu"
...
@torch.jit.script
def set_placeholder_root_pos(x, seq_slice_start:int,seq_slice_stop:int, midway_targets, p_slice_start:int,p_slice_stop:int):
    ...
    while epoch < config["train"]["total_epoch"]:
        for i, data in enumerate(data_loader, 0):
            ...
            keyframe_pos_idx = get_keyframe_pos_indices(
                window_len, seq_slice.start,seq_slice.stop, dtype, device)
            ...
            x = set_placeholder_root_pos(x, seq_slice.start,seq_slice.stop, midway_targets, p_slice.start,p_slice.stop)
...
def evaluate(model:ContextTransformer, positions, rotations, seq_slice:slice, indices:Dict[str,int],
             mean, std, atten_mask, post_process:bool=False,
             midway_targets:List[int]=(),geo=None):
    """
    ...
    with torch.no_grad():
        ...
        # calculate model output y
        traced_script_module = torch.jit.trace(model, (x,keyframe_pos_idx,atten_mask))
        traced_script_module.save(f"traced_model_ctx{window_len}.pt")
        model_out = model(x, keyframe_pos_idx, mask=atten_mask)
...
if __name__ == "__main__":
    scripted_fn1 = torch.jit.script(set_placeholder_root_pos)
    print(scripted_fn1.code)
    scripted_fn1.save("set_placeholder_root_pos.pt")
