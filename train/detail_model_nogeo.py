...
def train(config, context_config):
    ...
    while epoch < config["train"]["total_epoch"]:
        for i, data in enumerate(data_loader, 0):
            ...
            trans_len = random.choice([random.randint(min_trans, max_trans),min(frame_nums)-2])# random.randint(min_trans, max_trans)
            ...
            pos_ctx, rot_ctx = ctx_mdl.evaluate(
                ...
                post_process=False, midway_targets=midway_targets) # fixed: post-process
            ...
            state_out,c_out = get_model_output(
                ...
                trends, seq_slice, c_slice, rp_slice) # FIXME: 6
            ...
            rot_6d = train_utils.get_new_rotations6D(state_out,indices)
            foot_state=torch.cat([pos_new,rot_6d],dim=-1)
...
def eval_on_dataset(config, data_loader, detail_model, context_model,
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        pos_new, rot_new, _ = evaluate(
            detail_model, context_model, positions, rotations, trends,
...
def evaluate(detail_model, context_model, positions, rotations, foot_contact,
    ...
    with torch.no_grad():
        ...
        pos_ctx, rot_ctx = ctx_mdl.evaluate(
            context_model, positions, rotations, seq_slice,
            indices, mean_ctx, std_ctx, atten_mask_ctx,
            post_process=False, midway_targets=midway_targets)
