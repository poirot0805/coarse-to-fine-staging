...
def train(config):
    ...
    while epoch < config["train"]["total_epoch"]:
        for i, data in enumerate(data_loader, 0):
            ...
            # positions, rotations = data_utils.to_start_centered_data(positions, rotations, context_len) # BUG：不需要
            # global_rotations, global_positions = data_utils.fk_torch(rotations, positions, parents)
            global_rotations = rotations
            global_positions = positions

            # randomize transition length
            # min_frame=min(frame_nums)-2
            trans_len = random.randint(min_trans, max_trans) # random.choice([random.randint(min_trans, max_trans),min(frame_nums)-2])
            # print(f"trans:{trans_len}---minframe:{min_frame}")
            ...
            rot_6d = train_utils.get_new_rotations6D(y,indices)
...
def eval_on_dataset(config, data_loader, model, trans_len,
    ...
    for i, data in enumerate(data_loader, 0):
        ...
        #     positions, rotations, context_len)    # NOTICE:to_start_centered_data是否有必要，会不会影响结果
