...
from motion_inbetween_normal.model.rmi import (
...
def get_rmi_benchmark_stats(config, use_cache=True,
    ...
    if stats_folder is None:
        stats_folder = config["datasets"][dataset_name]["bvh_folder"]
    stats_path = os.path.join(
        stats_folder, "bench_stats_{}.pkl".format(context_len)
    )
    ...
    if use_cache and os.path.exists(stats_path):
        ...
        for i, data in enumerate(data_loader, 0):
            ...
            global_pos.append(positions.cpu().numpy())
...
def train(config):
    ...
    bench_dataset_names, _, bench_data_loaders = \
        train_utils.get_benchmark_datasets(config, device, shuffle=False)
